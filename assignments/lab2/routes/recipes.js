...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      let key = 'showRecipes' + page;
      await client.set(key, JSON.stringify(recipeList));
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      // store in the cache
      await client.set(newRecipe._id, JSON.stringify(newRecipe));
      // update scoreboard
      await client.ZADD("testSortList", {score:1, value: JSON.stringify(newRecipe)});
...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      if (getRecipe) {
        await client.set(req.params.id, JSON.stringify(getRecipe));
        let isExistInSortedSet = await client.zRank("testSortList", JSON.stringify(getRecipe));
        if (isExistInSortedSet != null) {
          await client.zIncrBy('testSortList', 1, JSON.stringify(getRecipe));
        } else {
          await client.ZADD("testSortList", {score:1, value: JSON.stringify(getRecipe)});
        }
      }
      res.json(getRecipe);
    } catch(e) {
  ...
  .patch(async (req, res) => {
    ...
    if (Object.keys(updatedObject).length !== 0) {
      try {
        ...
        // update the data store in the cache by using key
        await client.SET(updatedRecipe._id, JSON.stringify(updatedRecipe));

        // update score board
        let isExistInSortedSet = await client.zRank("testSortList", JSON.stringify(oldRecipes));
        let oldscore;
        if (isExistInSortedSet != null) {
          oldscore = await client.ZSCORE('testSortList', JSON.stringify(oldRecipes));
          await client.ZREM('testSortList', JSON.stringify(oldRecipes));
          await client.ZADD('testSortList', {score: oldscore + 1, value: JSON.stringify(updatedRecipe)});
        } else {
          await client.ZADD("testSortList", {score:1, value: JSON.stringify(updatedRecipe)});
        }

        return res.json(updatedRecipe);
      } catch (e) {
        if (e == 'You cannot update recipe with identical infomation') {
          return res.status(400).json({error:e});
        }
        return res.status(500).json({error: e});
      }
...
router.route('/:id/comments').post( async (req, res) => {
  ...
  // check the recipeId exist in database
  try {
    oldRecipes = await recipeData.getRecipeById(id);
  } catch (e) {
    return res.status(404).json({error: 'Recipe Not Found'});
  }
  ...
  try {
    ...
    // update the data store in the cache by using key
    await client.SET(returnRecipe._id, JSON.stringify(returnRecipe));
    ...
    if (isExistInSortedSet != null) {
      oldscore = await client.ZSCORE('testSortList', JSON.stringify(oldRecipes));
      await client.ZREM('testSortList', JSON.stringify(oldRecipes));
      await client.ZADD('testSortList', {score: oldscore + 1, value: JSON.stringify(returnRecipe)});
    } else {
      await client.ZADD("testSortList", {score:1, value: JSON.stringify(returnRecipe)});
    }
...
router.route('/:recipeId/:commentId').delete(async (req, res) => {
  ...
  try {
    ...
    // update the data store in the cache by using key
    await client.SET(recipeAfterDeleteComment._id, JSON.stringify(recipeAfterDeleteComment));
    ...
    if (isExistInSortedSet != null) {
      oldscore = await client.ZSCORE('testSortList', JSON.stringify(oldRecipes));
      await client.ZREM('testSortList', JSON.stringify(oldRecipes));
      await client.ZADD('testSortList', {score: oldscore + 1, value: JSON.stringify(recipeAfterDeleteComment)});
    } else {
      await client.ZADD("testSortList", {score:1, value: JSON.stringify(recipeAfterDeleteComment)});
    }
...
router.route('/:id/likes').post(async(req, res) => {
  ...
  try {
    ...
    // update the data store in the cache by using key
    await client.SET(recipeAterLike._id, JSON.stringify(recipeAterLike));
    ...
    if (isExistInSortedSet != null) {
      oldscore = await client.ZSCORE('testSortList', JSON.stringify(oldRecipes));
      await client.ZREM('testSortList', JSON.stringify(oldRecipes));
      await client.ZADD('testSortList', {score: oldscore + 1, value: JSON.stringify(recipeAterLike)});
    } else {
      await client.ZADD("testSortList", {score:1, value: JSON.stringify(recipeAterLike)});
    }
